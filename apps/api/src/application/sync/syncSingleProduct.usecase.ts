...
export class SyncSingleProductUsecase {
  ...
  async syncCardMarket(
    { name, cardMarketLink, keyword, blocked }: ProductEntity,
    page: Page
  ): Promise<
  ...
  > {
    ...
    try {
      await page.goto(
        cardMarketLink + "?language=1&minCondition=2&isSigned=N&isAltered=N",
        {
      ...
      if (keyword) {
        data = await page.$eval(
          "div.table-body",
          (table, { keyword, blocked }) => {
            const textContent = table.innerText.trim();
            ...
            for (let i = 0; i < elements.length; i += 5) {
              ...
              // if (
              //   blocked.length &&
              //   blocked.some((word: string) => description.includes(word))
              // ) {
              //   if (isKSeller) {
              //     i++;
              //   }
      ...
      // tmp = data;
      console.log("CardMarket data", data); // exemple: [ '3', 'K', 'menor-com', 'NM', 'nm+', '420,00 €', '1' ]
      ...
      console.log("No CardMarket listing", name);
      return undefined;
    }
  }
