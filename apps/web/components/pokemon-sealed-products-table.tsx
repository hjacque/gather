...
function createColumns(
  ...
  set?: boolean,
  block?: boolean,
): ColumnDef<GetProductsResponseItem>[] {
  return [
    {
      accessorKey: 'name',
      header: ({ column }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            {typeof row.original.market === 'number' ? fmt.format(row.original.market) : '-'}
          </div>
        );
      },
      enableSorting: true,
        ...
        return (
          <div>
            {typeof row.original.perBooster === 'number'
              ? fmt.format(row.original.perBooster)
              : '-'}
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            {typeof row.original.tcgp === 'number' ? fmt.format(row.original.tcgp) : '-'}
          </div>
        );
      },
      enableSorting: true,
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            {typeof row.original.fullSet === 'number' ? fmt.format(row.original.fullSet) : '-'}
          </div>
        );
      },
      enableSorting: true,
    ...
    {
      id: 'block',
      accessorFn: (row) => row.productSet.block,
      header: undefined,
      filterFn: (row, columnId, filterValue: string[]) => {
        if (!filterValue?.length) return true;
        return filterValue.includes(row.getValue(columnId));
      ...
      enableHiding: true,
      enableSorting: false,
      enableColumnFilter: block,
    },
  ];
}

export function PokemonSealedProductsTable({
  dataPromise,
  type,
  set,
  block,
  ...
  const columns = React.useMemo(() => createColumns(type, set, block), [type, set, block]);
  return (
    <ProductTable
      ...
      filters={
        type || set || block
          ? (data, table) => {
              const allTypes = [...new Set(data.map((i) => i.type))].sort();
              const allProductSets = [...new Set(data.map((i) => i.productSet.name))].sort();
              const allBlocks = [
                ...new Set(
                  data.filter((i) => !!i.productSet.block).map((i) => i.productSet.block!),
                ),
              ].sort();
              return (
                <div className="flex justify-items-start gap-4">
                  {type && <TypeMultiSelectFilter table={table} allTypes={allTypes} />}
                  {set && (
                    <ProductSetMultiSelectFilter table={table} allProductSets={allProductSets} />
                  )}
                  {block && <BlockMultiSelectFilter table={table} allBlocks={allBlocks} />}
                </div>
              );
            }
          : undefined
