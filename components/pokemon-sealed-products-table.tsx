...
export function PokemonSealedProductsTable({
  ...
  const columns: ColumnDef<z.infer<typeof getProductsResSchema>>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            {typeof row.original.fullSet === 'number'
              ? currencyFormatter.format(row.original.fullSet)
              : '-'}
          </div>
        );
      },
  ...
  const allBlocks = useMemo<string[]>(() => {
    const set = new Set(
      data
        .filter((item) => !!item.productSet.block)
        .map((item) => item.productSet.block!),
    );
    return Array.from(set).sort();
  }, [data]);
      ...
      >
        {(type || set || block) && (
          <div className="flex justify-items-start gap-4">
            ...
            {block && (
              <BlockMultiSelectFilter table={table} allBlocks={allBlocks} />
            )}
          </div>
        )}
